...
<div class="box">
    ...
    <a-form-model ref="ruleForm"  :rules="rules" :model="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }" @submit="handleSubmit" @submit.native.prevent class="form-wrap">
        <a-form-model-item label="文献类型" prop="type">
            <a-select v-model="form.type" prop="type" size="large" placeholder="文献类型">
                <a-select-option value="1">期刊-高级检索</a-select-option>
                <a-select-option value="2">期刊-专业检索</a-select-option>

                <a-select-option value="3">会议-高级检索</a-select-option>
                <a-select-option value="4">会议-专业检索</a-select-option>

                <a-select-option value="5">专利-高级检索</a-select-option>
                <a-select-option value="6">专利-专业检索</a-select-option>

                <a-select-option value="7">博硕士-高级检索</a-select-option>
                <a-select-option value="8">博硕士-专业检索</a-select-option>
            </a-select>
        </a-form-model-item>
        
        <a-form-model-item label="专题" prop="specialSubjectId">
            <a-select v-model="form.specialSubjectId" size="large" placeholder="专题">
                <a-select-option :value="item.institutionId" v-for="item in subjectList" :key="item.id">{{item.name}}</a-select-option>
            </a-select>
        </a-form-model-item>
